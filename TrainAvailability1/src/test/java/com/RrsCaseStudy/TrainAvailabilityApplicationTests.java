...
import com.RrsCaseStudy.Resource.TrainAvailabilityResource;
...
@SpringBootTest
class TrainAvailabilityApplicationTests {
	@Autowired
	TrainAvailabilityResource service;
	@MockBean
	...
	public void getTrainTest()
	{
		when(repository.findAll()).thenReturn(Stream
			.of(new TrainAvailability(12,"Tapan","Banglore","Delhi",8346.23d,null), new TrainAvailability(12,"Tapan","Banglore","Delhi",8346.23d,null))
			.collect(Collectors.toList()));
	assertEquals(2,service.getTrains().size());
	}
	
	@Test
	public void getTrainBySource()
	{
		String source = "Bangalore";
		when(repository.findByStartLocation(source)).thenReturn(Stream
				.of(new TrainAvailability(12,"Tapan","Banglore","Delhi",8346.23d,null))
				.collect(Collectors.toList()));
		assertEquals(1,service.getTrainByStartLocation(source).size());
	}
	@Test
	public void getTrainByDestination()
	{
		String destination = "Delhi";
		when(repository.findByStartLocation(destination)).thenReturn(Stream
				.of(new TrainAvailability(12,"Tapan","Banglore","Delhi",8346.23d,null))
				.collect(Collectors.toList()));
...
//		
	...
	public void deleteTrainTest()
	{
		...
		verify(repository,times(1)).delete(train);
